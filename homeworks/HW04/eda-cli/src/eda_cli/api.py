...
# Настраиваем логгер
logger = logging.getLogger("dataset_quality_api")
logger.setLevel(logging.INFO)

handler = logging.FileHandler(LOG_DIR / "api.log", encoding="utf-8")
handler.setLevel(logging.INFO)

# Простой формат (будем логировать JSON вручную)
...
@app.post(
    ...
    tags=["quality"],
    summary="Возвращает полный набор флагов качества из HW03",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict:
    ...
    start = perf_counter()  # ← добавили начало таймера
    ...
    if df.empty:
        raise HTTPException(
            status_code=400,
            detail="CSV-файл пустой (нет данных).",
        )
    ...
    # Приводим значения к JSON-friendly форматам
    serializable_flags = {
        key: (bool(val) if isinstance(val, (bool, int, float)) else val)
        for key, val in flags_all.items()
    }

    latency_ms = (perf_counter() - start) * 1000.0  # ← вычисление latency_ms
    ...
    return {"flags": serializable_flags}